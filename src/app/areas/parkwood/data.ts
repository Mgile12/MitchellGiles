import type { AreaPageData } from '@/lib/area-data';

export const areaData: AreaPageData = {
  name: 'Parkwood',
  slug: 'parkwood',
  seoTitle: 'Marketing Consultant Parkwood | Predictable Growth Systems | Gold Coast Expert',
  metaDescription:
    'Looking for a Marketing Consultant in Parkwood? Mitchell Giles builds data-driven marketing systems that generate consistent leads and measurable revenue for Gold Coast businesses.',
  heroH1:
    'Marketing Consultant Parkwood \u2013 Systems Built for Predictable Growth',
  heroIntro: [
    'Parkwood is in a unique position on the Gold Coast.',
    'You\u2019ve got residential estates. You\u2019ve got commercial pockets. You\u2019ve got proximity to The Club at Parkwood Village, Griffith University and the Gold Coast University Hospital precinct.',
    'That means opportunity.',
  ],
  whyDifferentTitle: 'Why Parkwood Businesses Need Structured Marketing',
  whyDifferentIntro: [
    'Parkwood sits between Southport and Arundel, with easy access via Olsen Avenue.',
  ],
  whyDifferentPoints: [
    'Parkwood search patterns require suburb-level targeting',
    'Local businesses depend on search visibility and referrals',
    'Seasonal demand shifts affect marketing effectiveness',
    'Competition from nearby suburbs requires differentiation',
    'Paid ads fluctuate without proper tracking and optimisation',
    'Websites generating traffic but not converting leads',
  ],
  whyChooseLocalAuthority: [
    'No generic packages',
    'No tiered pricing games',
    'Custom-built revenue systems',
    'Australian-focused strategies',
  ],
  servicesIntro:
    'Structured marketing systems designed for Parkwood businesses where search visibility and reputation drive growth.',
  seoTargets: [
    'Marketing Consultant Parkwood',
    'Best Marketing Consultant in Parkwood',
  ],
  ctaNearbyPlaces: [
    'Parkwood',
    'Parkwood local area',
    'Broadwater Parklands',
    'Australia Fair',
  ],
  directions: [
    'Head toward the nearest major road connecting to Southport',
    'Follow signs toward the Gold Coast Highway',
    'Continue toward Marine Parade, Southport',
    'Arrive at 82 Marine Parade',
    'Approximately 20\u201325 minutes by car',
  ],
  publicTransport: [
    'Bus connections available to Southport',
    'G:link light rail accessible via connecting services',
  ],
  landmarks: [
    { name: 'Parkwood', time: '20\u201325 minutes drive' },
    { name: 'Parkwood local area', time: '20\u201325 minutes drive' },
    { name: 'Broadwater Parklands', time: '3 min walk' },
    { name: 'Australia Fair', time: '4 min drive' },
  ],
  faqItems: [
    {
      question: 'How much does a marketing consultant in Parkwood cost?',
      answer:
        'Investment depends on your business goals and channels required. Most Parkwood businesses invest between $2,000 and $8,000 per month for a structured marketing system.',
    },
    {
      question: 'Are you based in Parkwood?',
      answer:
        'Our office is at 82 Marine Parade, Southport QLD 4215, servicing Parkwood and the wider Gold Coast.',
    },
    {
      question: 'How quickly will I see results?',
      answer:
        'Paid channels like Google Ads can generate leads within weeks. SEO and content systems typically show compounding results within 90 to 120 days.',
    },
    {
      question: 'What makes your approach different?',
      answer:
        'The focus is on structured systems that generate predictable results, not one-off campaigns. Every channel compounds with transparent reporting.',
    },
    {
      question: 'Can you help with lead generation for Parkwood businesses?',
      answer:
        'Lead generation systems deliver consistent, qualified enquiries through SEO, Google Ads, email marketing, and automation tailored to Parkwood.',
    },
    {
      question: 'Do you offer SEO services for Parkwood?',
      answer:
        'Yes. Local SEO is essential for Parkwood businesses competing for suburb-level search visibility.',
    },
    {
      question: 'Is reporting included?',
      answer:
        'Full transparency. Every lead, every cost, every result visible in your dashboard.',
    },
    {
      question: 'How do I get started?',
      answer:
        'Book a strategy session covering your current performance, identifying gaps, and outlining a plan for predictable growth in Parkwood.',
    },
  ],
  images: [
    {
      src: 'https://images.pexels.com/photos/3182812/pexels-photo-3182812.jpeg?auto=compress&cs=tinysrgb&w=800',
      alt: 'Marketing Consultant Parkwood QLD',
    },
    {
      src: 'https://images.pexels.com/photos/3184465/pexels-photo-3184465.jpeg?auto=compress&cs=tinysrgb&w=800',
      alt: 'Business growth near The Club at Parkwood Village',
    },
    {
      src: 'https://images.pexels.com/photos/3184339/pexels-photo-3184339.jpeg?auto=compress&cs=tinysrgb&w=800',
      alt: 'Gold Coast marketing consultant Southport office',
    },
    {
      src: 'https://images.pexels.com/photos/3184357/pexels-photo-3184357.jpeg?auto=compress&cs=tinysrgb&w=800',
      alt: 'Local SEO services Parkwood',
    },
  ],
  internalLinks: [
    { href: '/seo-gold-coast', label: 'SEO Gold Coast' },
    { href: '/google-ads-gold-coast', label: 'Google Ads Gold Coast' },
    { href: '/email-marketing-gold-coast', label: 'Email Marketing Gold Coast' },
    { href: '/lead-generation-gold-coast', label: 'Lead Generation Gold Coast' },
    { href: '/marketing-automation-gold-coast', label: 'Marketing Automation Gold Coast' },
  ],
};
